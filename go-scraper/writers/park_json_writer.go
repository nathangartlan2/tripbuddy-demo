...
// ParkJSONWriter subscribes to park events and writes them to JSON files
type ParkJSONWriter struct {
	outputDir string
}

// NewParkJSONWriter creates a new JSON writer that writes to the specified output directory
func NewParkJSONWriter(outputDir string) *ParkJSONWriter {
	return &ParkJSONWriter{
		outputDir: outputDir,
	}
}

// OnParkScraped is called when a park is scraped - writes it to a JSON file
func (w *ParkJSONWriter) OnParkScraped(event events.ParkScrapedEvent) {
	if event.Park == nil {
...
func (w *ParkJSONWriter) generateFilename(parkName string) string {
